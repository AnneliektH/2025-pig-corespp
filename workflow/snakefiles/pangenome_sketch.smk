# Snakefile for processing pangenomes from a list of species
# -----------------------------------
# This workflow takes a list of species from a TSV file,
# gets their sourmash signature from either GTDB or GTDB+our pig-specific bins, 
# makes a pangenome sketch (at k21 and k31), and compares the two to each other. 
# How many new kmers do our pig specific MAGs add to these species pangenomes
# Output files from snakefile are used in the branchwater.smk workflow
#
# output files are in the /results/pangenome directory
# 
# Author: Anneliek ter Horst
# Date: 2025-11-06
...
SUFFIXES = ["gtdb.k31.pang", "gtdb+mags.k31", "gtdb+mags.k31.novel_content"]
...
# How much novel content does our pangenome have vs gtdb
# Subtract the GTDB content from all content (what is new)
rule subtract:
    input:
        sig_gtdb_k31 = f"{PANG_OUT}/{{pang_folder}}/{{pang_folder}}.gtdb.k31.pang.zip",
        sig_all_k31 = f"{PANG_OUT}/{{pang_folder}}/{{pang_folder}}.gtdb+mags.k31.zip",
    output:
        sig_gtdb_k31 = temp(f"{PANG_OUT}/{{pang_folder}}/{{pang_folder}}.gtdb.k31.merged.zip"),
        sig_all_k31 = temp(f"{PANG_OUT}/{{pang_folder}}/{{pang_folder}}.gtdb+mags.k31.merged.zip"),
        sig_sub = f"{PANG_OUT}/{{pang_folder}}/{{pang_folder}}.gtdb+mags.k31.novel_content.zip",
    conda: "branchwater-skipmer"
    threads: 1
    shell:
        """
        sourmash sig merge {input.sig_all_k31} --flatten -k 31 -o {output.sig_all_k31} && \
        sourmash sig merge {input.sig_gtdb_k31} --flatten -k 31 -o {output.sig_gtdb_k31} && \
        sourmash sig subtract {output.sig_all_k31} {output.sig_gtdb_k31} -o {output.sig_sub} -k 31
        """

# get signature sizes
rule get_sig_size:
    input:
        f"{PANG_OUT}/{{pang_folder}}/{{pang_folder}}.{{suffix}}.zip"
    output:
        temp(f"{PANG_OUT}/{{pang_folder}}/{{pang_folder}}.{{suffix}}.size.txt")
    conda: "branchwater-skipmer"
    shell:
        """
...
rule aggregate_hash_sizes:
    input:
        lambda wildcards: expand(
            f"{PANG_OUT}/{{pang_folder}}/{{pang_folder}}.{{suffix}}.size.txt",
            pang_folder=wildcards.pang_folder,
            suffix=SUFFIXES
        )
    output:
        f"{PANG_OUT}/{{pang_folder}}/{{pang_folder}}.sizes.tsv"
    run:
        sizes = {}
